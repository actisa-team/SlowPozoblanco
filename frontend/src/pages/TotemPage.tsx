...
export const TotemPage = () => {
    ...
    return (
        <div className="h-screen lg:h-screen w-full bg-app overflow-y-auto overflow-x-hidden lg:overflow-hidden font-sans text-main relative">
            <div className="flex flex-col lg:grid lg:grid-cols-12 h-max lg:h-full gap-4 lg:p-6 p-4 box-border lg:portrait:grid-rows-12 lg:landscape:grid-rows-[auto_auto_1fr_auto_auto]">
                ...
                <section className="col-span-12 lg:portrait:row-span-6 lg:landscape:row-start-3 relative group min-h-[45vh] lg:min-h-0 overflow-hidden shrink-0 lg:h-full">
                    {allMedia.length > 0 ? (
                        <div className="absolute inset-0 flex items-center justify-center gap-4">
                            {/* Previous Item */}
                            <div 
                                className="hidden lg:block h-[90%] w-64 bg-gray-300 rounded-3xl opacity-70 transform scale-95 -translate-x-2 overflow-hidden cursor-pointer hover:opacity-100 transition-opacity"
                                onClick={handlePrev}
                            >
                ...
                <div className="col-span-12 lg:col-span-9 lg:portrait:row-span-1 lg:landscape:row-start-2 overflow-x-auto no-scrollbar pb-2 lg:pb-0 lg:h-full">
                    <div className="flex items-center gap-2 lg:gap-4 px-2 lg:px-4 h-16 lg:h-full lg:landscape:h-24">
                        {IMAGES.dots.map((src, i) => (
                            <div key={i} className="h-10 lg:h-full aspect-square rounded-full border-2 lg:border-4 border-white shadow-md lg:shadow-lg overflow-hidden shrink-0">
                                <img src={src} alt="" className="h-full w-full object-cover" />
                            </div>
                        ))}
                        <div className="h-[1px] bg-muted/20 flex-1 ml-4 hidden lg:block" />
                    </div>
                </div>

                <div className="col-span-12 lg:col-span-3 lg:portrait:row-span-1 lg:landscape:row-start-2 lg:h-full">
                    <AudioguideWidget />
                </div>
                ...
                <div className="col-span-12 lg:col-span-9 lg:portrait:row-span-3 lg:landscape:row-start-4 grid grid-cols-2 sm:grid-cols-3 gap-2 lg:gap-4">
                    {CATEGORIES.map((cat) => (
                        <Card
                            key={cat.id}
                            className="relative overflow-hidden group p-0 cursor-pointer active:scale-95 transition-transform h-24 sm:h-32 lg:h-auto lg:h-full shrink-0"
                        ...
                        >
                            ...
                            <div className="absolute inset-0 bg-black/50 group-hover:bg-black/30 transition-colors flex items-center justify-center p-2">
                                <h3 className="text-white font-bold text-sm sm:text-base lg:text-xl text-center break-words drop-shadow-md">{cat.label}</h3>
                            </div>
                        </Card>
                    ))}
                </div>
                ...
                <div className="col-span-12 lg:col-span-3 lg:portrait:row-span-3 lg:landscape:row-start-4 flex flex-col sm:flex-row lg:flex-col gap-4 lg:landscape:flex-row sm:landscape:flex-row">
                ...
                {/* --- F. Footer --- */}
                <footer className="col-span-12 lg:portrait:row-span-1 lg:landscape:row-start-5 -mx-6 -mb-6 mt-4 lg:mt-2 pb-6 lg:pb-0">
                    <Ticker />
                </footer>
